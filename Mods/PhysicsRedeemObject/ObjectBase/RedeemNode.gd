...
func set_redeem_rigid_body(redeem_rigid_body: RedeemRigidBody):
	...
	if redeem_rigid_body.look_at_avatar:
		redeem_rigid_body.look_at(target_position, Vector3(0,1,0), true)
...
func _ready():
	...
	var main_child:RedeemRigidBody = null
	for child in get_children():
		if child is RedeemRigidBody:
			main_child = child
	if main_child == null:
		printerr ("RedeemNode: RedeemObjects does not contain expected RedeemRigidBody! ")
		queue_free()
		return
		
	set_redeem_rigid_body(main_child)
